class AitrixLab {
    ...
    generateScenarioCards() {
        ...
        return scenarios.map(scenario => `
            <div class="scenario-card" id="scenario-${scenario.id}" data-answer="${scenario.answer}">
                <h5>${scenario.text}</h5>
                <p>Click to select, then choose OS →</p>
            </div>
        `).join('');
    }
    ...
    generateOSCards() {
        ...
        return systems.map(os => `
            <div class="os-card" id="os-${os.id}" data-os="${os.id}">
                <h5>${os.name}</h5>
                <p>${os.desc}</p>
            </div>
        `).join('');
    ...
    setupMatchingInteractions() {
        // OS selection buttons are handled via onclick attributes in HTML
        console.log('Matching interactions set up');
    }
...
DOWNLOADED: 4612 - FOUND: 4`;
    ...
    // Complete implementations for advanced challenges
    generateNetworkDesignInterface() {
        return `
            <div class="challenge-interface active">
                <div class="scenario-panel">
